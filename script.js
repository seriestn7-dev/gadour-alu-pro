...
window.goToPricesAndClose = function() { closeWelcomePopup(); switchMode('prices'); }
...
function generateCutData(calculateMetersOnly = false) {
    ...
    for (const it of devis) {
        ...
        if(it.hasFix) {
            if(it.fixPos === 'top' || it.fixPos === 'bottom') { finalH = H - it.fixSize - EPAISSEUR_TRAVERSE; addPiece("p_Traverse40104", L - Q_TRAVERSE, 1 * Q); addPiece("p_40166", it.fixSize - 2, 2 * Q); addPiece("p_40166", L - 2, 2 * Q); } 
            else { finalL = L - it.fixSize - EPAISSEUR_TRAVERSE; addPiece("p_Traverse40104", H - Q_TRAVERSE, 1 * Q); addPiece("p_40166", it.fixSize - 2, 2 * Q); addPiece("p_40166", H - 2, 2 * Q); }
        }
        ...
        if (it.product === "monobloc") {
            ...
            let hFarda = finalH - 4.2; let wFarda = is2V ? ((finalL - 4.5) / 2) : (finalL - 4.2);
            addPiece("p_40402", H+7, 2*Q); addPiece("p_40402", L+7, 2*Q);
            addPiece(profilOuvrant, hFarda, (is2V ? 4 : 2) * Q); addPiece(profilOuvrant, wFarda, (is2V ? 4 : 2) * Q);
    ...
    for (let ref in piecesNeeded) {
        ...
        let bars = [];
        cuts.forEach(c => {
            let placed = false; for(let b of bars) { if(b.rem >= (c + CUT_MARGIN)) { b.cuts.push(c); b.rem -= (c + CUT_MARGIN); placed = true; break; } }
...
window.calculateTotalDevis = function() {
    ...
    for (const it of devis) {
        ...
        if(it.hasFix) {
             if(it.fixPos === 'top' || it.fixPos === 'bottom') { workingH = H - it.fixSize - EPAISSEUR_TRAVERSE; mat.a_Joint_Vitrage_242 += (((L + it.fixSize)*2)/100) * Q; } 
             else { workingL = L - it.fixSize - EPAISSEUR_TRAVERSE; mat.a_Joint_Vitrage_242 += (((H + it.fixSize)*2)/100) * Q; }
        }
    ...
    html += `<div style="text-align:right; margin-top:20px;"><h2 style="color:#d32f2f; border-top:2px solid #333; padding-top:10px;">TOTAL NET: ${grandTotal.toFixed(3)} TND</h2>`;
    if(margePercent > 0) html += `<h2 style="color:#28a745;">TOTAL AVEC MARGE (+${margePercent}%): ${finalTotal.toFixed(3)} TND</h2>`;
    html += `</div>`;
...
function updateDarkModeIcon() {
    ...
    if(btn) btn.innerText = document.body.classList.contains('dark-mode') ? '☀️ Mode Jour' : '🌙 Mode Nuit';
}

document.addEventListener('DOMContentLoaded', () => { loadPrices(); updateUI(); loadTheme(); });
